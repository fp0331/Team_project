...
int LSM6DSOXClass::begin()
{
  ...
  writeRegister(LSM6DSOX_CTRL2_G, 0x6C); //modificato, prima era 0x4C

  // Set the Accelerometer control register to work at 104 Hz, 4 g,and in bypass mode and enable ODR/4
  // low pass filter (check figure9 of LSM6DSOX's datasheet)
  writeRegister(LSM6DSOX_CTRL1_XL, 0x6A); //modificato, prima era 0x4A
...
{
    if (readRegister(LSM6DSOX_CTRL1_XL)==0x6A){
        return 416.0F;
    }
    else if (readRegister(LSM6DSOX_CTRL1_XL)==0x4A){
        return 104.0F;
    }
}
...
{
    if (readRegister(LSM6DSOX_CTRL2_G)==0x6C){
        return 416.0F;
    }
    else if (readRegister(LSM6DSOX_CTRL2_G)==0x4C){
        return 104.0F;
    }
}
